...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('superadmin.taxes.update', $taxe->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <select name="mairie_ref" class="form-select" id="mairieSelect"
                                data-url="{{ route('superadmin.taxes.infos.mairie', ['id' => 'ID_PLACEHOLDER']) }}">
                                <option value="">-- Choisissez une mairie --</option>
                                @foreach ($mairies as $mairie)
                                    <option value="{{ $mairie->mairie_ref }}"
                                        {{ old('mairie_ref', $taxe->mairie_ref) == $mairie->mairie_ref ? 'selected' : '' }}>
                                        {{ $mairie->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row" id="mairieInfos">
                        <div class="mb-3 col-md-6">
                            ...
                            <input type="text" class="form-control" id="region"
                                value="{{ $taxe->mairie->region->nom ?? '' }}" disabled>
                        </div>
                        <div class="mb-3 col-md-6">
                            <label for="commune">Commune</label>
                            <input type="text" class="form-control" id="commune"
                                value="{{ $taxe->mairie->commune->commune ?? '' }}" disabled>
                        </div>
                    </div>
        ...
        <div class="card-table-container mt-4">
            <h4 class="mb-4">Liste des mairies ayant des taxes assignées</h4>
            <div class="table-responsive">
                <table class="table custom-table" id="mairieTable" data-url="{{ route('superadmin.taxes.mairie.list') }}">
                    <thead>
                        <tr>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#mairieTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('superadmin.taxes.mairie.list') }}", // Assurez-vous que ce nom de route est correct
                columns: [{
                        data: 'name',
                        name: 'name'
                    },
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    alert(
                        "Une erreur est survenue lors du chargement des données. Veuillez vérifier la console du navigateur.");
                }
            });
        });
    </script>
